...
- Integrated notes — monthly memos by default, switches to range-specific notes when a date range is selected
...
When no range is selected, the notes area stores monthly memos. When a range is selected, it switches to range-specific notes. Both are stored independently so nothing gets overwritten — this felt like the most intuitive UX.
...
Tested on Chrome, Firefox, and Safari. Uses standard CSS features — no vendor prefixes needed.
